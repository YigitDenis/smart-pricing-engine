...
try:
    ...
    for product_id, group in df_sorted.groupby('Id_clean'):
        ...
        avg_weekly_sales = round(total_sales / weeks_in_sales, 1) if weeks_in_sales > 0 else 0.0
        ...
        urun_yasam_dongusu = "Yeni Ürün (≤4 Hafta) 🚀" if weeks_in_sales <= 4 else "Aktif / Olgun Ürün 📈"
        ...
        true_weekly_sales = true_weekly_sales + (lost_sales / weeks_in_sales if weeks_in_sales > 0 else 0)
        ...
        running_cum_sales = 0
        for _, row_item in group_sorted.iterrows():
            w_name = str(row_item.get('Hafta', '1.Hafta')).strip()
            w_satis = float(row_item.get('Satis', 0))
            w_ciro = float(row_item.get('Ciro', 0))
            w_raw_stock = float(row_item.get('Stok_Data', 0))

            running_cum_sales += int(w_satis)
            ...
            agg_rows.append({
                ...
                'Hafta': w_name,
                'Haftalık Satış': int(w_satis),
                'Kümülatif Satış': running_cum_sales,
                'Haftalık Ciro': w_ciro,
                'Haftalık Stok': int(round(w_raw_stock)),
                'Kaç Haftadır Satışta': weeks_in_sales,
    ...
    unique_weeks_sorted = sorted(df_summary['Hafta'].dropna().unique().tolist(), key=lambda x: extract_week_num(x))
    ...
    if selected_week != 'Tümü':
        df_filtered = df_filtered[df_filtered['Hafta'] == selected_week]
    ...
    tot_satis = int(df_filtered['Haftalık Satış'].sum())
    tot_ciro = float(df_filtered['Haftalık Ciro'].sum())
    tot_stok = int(df_filtered.drop_duplicates(subset=['ID'])['Güncel Stok (Genel Depo)'].sum())
    
    df_filtered_unique_id = df_filtered.drop_duplicates(subset=['ID'])
    tot_lost_sales = int(df_filtered_unique_id['Satış Kaybı Adet'].sum())
    tot_lost_amount = float(df_filtered_unique_id['Satış Kaybı Tutarı (TL)'].sum())
    ...
    with tab1:
        ...
        df_unique_summary = df_filtered.drop_duplicates(subset=['ID']).copy()
        ...
        display_cols_t1 = [
            ...
            'Toplam Satış', 'Toplam Ciro', 'Güncel Stok (Genel Depo)', 'ST %', 
            'OBF (Birim Fiyat)', 'Satış Kaybı Adet', 'Satış Kaybı Tutarı (TL)', 
            'Haftalık Ortalama Satış Adeti', 'Ürün İçeriği', 'Kumaş Dokusu'
        ]
        available_t1_cols = [c for c in display_cols_t1 if c in df_unique_summary.columns]
        
        styled_df1 = df_unique_summary[available_t1_cols].style.apply(
            highlight_inspected_row, axis=1
        ).map(
            highlight_sales_stock, subset=['Toplam Satış'] if 'Toplam Satış' in available_t1_cols else []
        ).map(
    ...
    with tab2:
        ...
        top_10_sales = df_filtered.drop_duplicates(subset=['ID']).sort_values(by='Toplam Satış', ascending=False).head(10)
        
        t2_cols = ['ID', 'Ürün Kodu', 'Ürün Adı', 'Renk Kodu', 'Renk', 'Anakategori', 'Deniz Açıklama', 'Toplam Satış', 'Güncel Stok (Genel Depo)', 'Mevcut Stok Kaç Hafta Gider (WOS)', 'Ürün Yaşam Döngüsü', 'Ürün Sezonu', 'RPT Kararı']
        ...
        styled_top10_sales = top_10_sales[available_t2_cols].style.apply(
            ...
            highlight_sales_stock, subset=['Toplam Satış']
        ).map(
            highlight_stock, subset=['Güncel Stok (Genel Depo)']
        ).format(format_dict)
    ...
    with tab3:
        ...
        top_10_stock = df_filtered.drop_duplicates(subset=['ID']).sort_values(by='Güncel Stok (Genel Depo)', ascending=False).head(10)
        
        t3_cols = ['ID', 'Ürün Kodu', 'Ürün Adı', 'Renk Kodu', 'Renk', 'Anakategori', 'Deniz Açıklama', 'Güncel Stok (Genel Depo)', 'Toplam Satış', 'Mevcut Stok Kaç Hafta Gider (WOS)', 'Ürün Yaşam Döngüsü', 'Ürün Sezonu', 'RPT Kararı']
    ...
    with tab4:
        ...
        auto_rpt = df_filtered[df_filtered['RPT Kararı'] == 'Acil RPT Gerekli 🚀'].drop_duplicates(subset=['ID']).sort_values(by='Önerilen RPT Adeti', ascending=False)
        st.metric("Acil RPT Verilmesi Gereken Renk Varyant Adedi", len(auto_rpt))
        
        rpt_display_cols = ['ID', 'Ürün Kodu', 'Ürün Adı', 'Renk Kodu', 'Renk', 'Anakategori', 'Deniz Açıklama', 'RPT Kararı', 'Önerilen RPT Adeti', 'Mevcut Stok Kaç Hafta Gider (WOS)', 'Haftalık Ortalama Satış Adeti', 'Güncel Stok (Genel Depo)', 'Toplam Satış', 'Ürün Yaşam Döngüsü', 'Ürün Sezonu', 'Ürün İçeriği', 'Kumaş Dokusu']
    ...
    with tab5:
        ...
        df_weekly_trend = df_filtered.groupby('Hafta').agg({
            'Haftalık Satış': 'sum',
            'Haftalık Ciro': 'sum',
            'Haftalık Stok': 'sum',
            'Haftalık Ortalama Satış Adeti': 'count'
        }).reset_index()
        
        df_weekly_trend['Hafta_Num'] = df_weekly_trend['Hafta'].apply(extract_week_num)
        df_weekly_trend = df_weekly_trend.sort_values(by='Hafta_Num').drop(columns=['Hafta_Num', 'Haftalık Ortalama Satış Adeti'])
        df_weekly_trend = df_weekly_trend.rename(columns={'Haftalık Satış': 'Toplam Hafta Satışı', 'Haftalık Ciro': 'Toplam Hafta Cirosu', 'Haftalık Stok': 'Toplam Hafta Stoku'})
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        df_weekly_trend.to_excel(writer, index=False, sheet_name='Hafta_Bazli_Trend')
        df_summary.to_excel(writer, index=False, sheet_name='Tum_Detaylar')
