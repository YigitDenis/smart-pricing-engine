...
def calculate_smart_pricing(df):
  ...
  # Aktif Hafta ve Satış Hızı Hesabı (Geçmiş haftalar biriktiğinde çalışır)
...
def load_and_aggregate_data(url):
  ...
  # Eğer birden fazla hafta varsa Id bazlı kümüle yapar, tek hafta varsa hata vermeden geçer
  if id_col and id_col in df.columns and "Hafta" in df.columns:
    # Birden fazla hafta olup olmadığını kontrol et
    if df["Hafta"].nunique() > 1:
      ...
      df_grouped = df.drop(columns=["Hafta"], errors="ignore")
  else:
    df_grouped = df.drop(columns=["Hafta"], errors="ignore")
...
try:
  ...
  id_col = (
      ...
      else ("ID" if "ID" in df_grouped.columns else None)
  )
  filter_col = (
      id_col if id_col else ("Ürün Kodu" if "Ürün Kodu" in df_grouped.columns else "Ürün Adı")
  )
  ...
  if filter_col and filter_col in df_grouped.columns:
    ...
    if selected_code != "Tümü":
      df_filtered = df_grouped[
          df_grouped[filter_col].astype(str) == str(selected_code)
      ].copy()
    else:
